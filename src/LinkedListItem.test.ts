...
describe("LinkedListItem~insertBefore", () => {
	test("inserts given LinkedListItem before this", () => {
		const baseItem = new LinkedListItem(1);
		const itemBefore = new LinkedListItem(0);
		baseItem["insertBefore"](itemBefore);
		expect(baseItem.before).toBe(itemBefore);
	});

	test("adds cleanup function of given item", () => {
		const baseItem = new LinkedListItem(1);
		let callbackCalled = false;
		const itemBefore = new LinkedListItem(0, () => { callbackCalled = true });
		baseItem["insertBefore"](itemBefore);
		baseItem.unlink();
