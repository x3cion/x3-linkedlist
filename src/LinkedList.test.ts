 ...
 */
function getListWithItems(count: number, offset: number = 0): { list: LinkedList<string>, items: LinkedListItem<string>[], values: string[] } {
...
function getValueOnIndex(list: LinkedList<string>, index: number): string {
	...
	if (!current) throw new Error("Index number yields no entry!");
	return current && current.value;
}

function getCallbackThisArgTest(targetedFunction: string): () => void {
	interface CallbackBasedFunctionReturningObject {
		[key: string]: (callback: () => any, thisArg: any) => any;
	}

	return () => {
		const { list: tmpList, items, values } = getListWithItems(1);
		const list = <CallbackBasedFunctionReturningObject><unknown>tmpList;
		const newThis1 = { testvalue: 1 };
		let callbackBeenCalled1 = false;
		list[targetedFunction](
			function (this: any) {
		...
		list[targetedFunction](function (this: any) {
...
describe("Helper functions are working", () => {
	test("getListWithItems returns valid pairs of list, items and values", () => {
		...
		expect(expectedValues.every(value => expectedValues.includes(value))).toBeTrue();
...
describe("LinkedList~getItemByIndex", () => {
	test("undefined index parameter throws", () => {
		const { list, items } = getListWithItems(2);
		expect(() => (<any>list)["getItemByIndex"]()).toThrow();
	});

	test("0 index parameter on empty list returns undefined", () => {
		let { list: list1, items: items1 } = getListWithItems(0);
		expect(list1["getItemByIndex"](0)).toBeUndefined();

		let { list: list2, items: items2 } = getListWithItems(1);
		list2.pop();
		expect(list2["getItemByIndex"](0)).toBeUndefined();
	});

	test("negative index parameter counts from end", () => {
		const { list, items } = getListWithItems(5);
		expect(list["getItemByIndex"](-0)).toBe(items[0]);
		expect(list["getItemByIndex"](-1)).toBe(items[4]);
		expect(list["getItemByIndex"](-3)).toBe(items[2]);
		expect(list["getItemByIndex"](-5)).toBe(items[0]);
		expect(list["getItemByIndex"](-6)).toBeUndefined();
	});

	test("positive index parameter counts from start", () => {
		const { list, items } = getListWithItems(5);
		expect(list["getItemByIndex"](0)).toBe(items[0]);
		expect(list["getItemByIndex"](1)).toBe(items[1]);
		expect(list["getItemByIndex"](3)).toBe(items[3]);
		expect(list["getItemByIndex"](5)).toBeUndefined();
	});
});

describe("LinkedList~unlinkCleanup", () => {
	test("removes first and last reference and counts list.length down", () => {
		const { list, items, values } = getListWithItems(2);
		...
		list["unlinkCleanup"](first);
		expect(list.length).toBe(1);
		expect(list.first).toBe(last);
		expect(list.last).toBe(last);

		// We have to cleanup the LinkedListItem here. This function is not meant to be called directly
		first["unlinkCleanup"] = undefined;
		first.unlink();

		list["unlinkCleanup"](last);
		expect(list.length).toBe(0);
		expect(list.first).toBeUndefined();
		expect(list.last).toBeUndefined();
	});
...
describe("LinkedList#map", () => {
	test("maps for every item in list", () => {
		...
		const output = list.map((value) => parseInt(value) + 1);
...
describe("LinkedList#reduce", () => {
	...
	test("works with given initialValue", () => {
		...
		const finalValue = list.reduce((acc, value) => acc + parseInt(value), -100);
		expect(finalValue).toBe(-94);
	});

	test("throws on empty list with empty accumulator", () => {
		const list = new LinkedList();
		expect(() => list.reduce(() => { })).toThrowError(TypeError);
	});
...
describe("LinkedList#reduceRight", () => {
	...
	test("works with given initialValue", () => {
		const { list } = getListWithItems(3, 1);
		const finalValue = list.reduceRight<string[]>((acc, value) => { acc.push(value); return acc; }, []);
		expect(finalValue).toEqual(["3", "2", "1"]);
	});

	test("throws on empty list with empty accumulator", () => {
		const list = new LinkedList();
		expect(() => list.reduceRight(() => { })).toThrowError(TypeError);
	});
...
describe("LinkedList#concat", () => {
	...
	test("joins pure LinkedList's together", () => {
		...
		for (const value of joinedList1.values()) {
			expect(list1.includes(value) || list2.includes(value) || list3.includes(value)).toBeTrue();
		}
	...
	test("joins abitrary items together", () => {
		...
		const joinedList = list1.concat<any>(string, list2, object, array, list1);
		expect(getValueOnIndex(joinedList, 4)).toBe(items2[1].value);
		expect(getValueOnIndex(joinedList, 5)).toBe(object);
...
describe("LinkedList#entries,[Symbol.iterator]", () => {
	test("returns exactly the content of the list", () => {
		...
		expect(iteratorResponse.every(([item, value]) => items.includes(item) && values.includes(value))).toBeTrue();

		expect(entriesResponse.length).toBe(4);
		expect(entriesResponse.every(([item, value]) => items.includes(item) && values.includes(value))).toBeTrue();
	});
